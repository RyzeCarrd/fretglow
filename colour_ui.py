"""Full-spectrum colour picker for every fret and the press effect."""
import colorsys
...
class ColourPicker:
    def __init__(self,app,colour,title,callback,palette):
        ...
        w.title(title);w.geometry('360x530');w.resizable(False,False);w.transient(app.root);w.configure(fg_color=palette['bg']);set_icon(w)
        ctk.CTkLabel(w,text=title,font=('Segoe UI',20,'bold'),text_color=palette['text']).pack(anchor='w',padx=24,pady=(20,0))
        ctk.CTkLabel(w,text='Pick any colour.',font=('Segoe UI',12),text_color=palette['muted']).pack(anchor='w',padx=24,pady=(0,10))
        ...
        colours=ctk.CTkFrame(w,fg_color='transparent');colours.pack(fill='x',padx=24,pady=(12,0))
        for name,value in [('Red','#FF405A'),('Orange','#FF9438'),('Yellow','#FFE34C'),('Green','#30DF72'),('Blue','#428AFF'),('Purple','#8B5CF6'),('White','#FFFFFF'),('Black','#000000')]:
            ctk.CTkButton(colours,text='',width=31,height=24,corner_radius=5,fg_color=value,hover_color=value,border_width=1,border_color=palette['line'],command=lambda colour=value:self.select_colour(colour)).pack(side='left',padx=(0,8))
        ctk.CTkLabel(w,text='Light / dark',text_color=palette['muted'],font=('Segoe UI',12)).pack(anchor='w',padx=24,pady=(8,0))
        self.light=ctk.CTkSlider(w,from_=0,to=1,command=self.set_value,progress_color=palette['accent'],button_color=palette['accent'],button_hover_color=palette['accent_hover'],fg_color=palette['line']);self.light.set(self.v);self.light.pack(fill='x',padx=24,pady=(4,12))
    ...
    def draw(self):
        # Keep the full rainbow visible even when the current colour is black.
        pixels=[tuple(round(c*255) for c in colorsys.hsv_to_rgb(x/309,1-y/189,1)) for y in range(190) for x in range(310)]
    ...
    def marker(self):
        self.canvas.delete('marker');x=self.h*309;y=(1-self.s)*189
    ...
    def choose(self,event):
        self.h=max(0,min(1,event.x/309));self.s=max(0,min(1,1-event.y/189))
        if self.v==0:self.v=1;self.light.set(1)
        self.update()
    def set_value(self,value):self.v=value;self.update()
    def select_colour(self,colour):self.hex.set(colour);self.from_hex()
    def from_hex(self,event=None):
        ...
        self.h,self.s,self.v=colorsys.rgb_to_hsv(*rgb);self.light.set(self.v);self.update();return True
    def accept(self):
